...
### 스와이프/뒤로가기로 앱 내부 화면 닫기 (2026-08-01, 커미션 완료 · 채팅 예정)
...
- **검증**(dev 브라우저): 목록→상세→후기 진입 시 스택이 `cmList>cmDetail>cmReviews`로 쌓이고, `history.back()` 두 번에 후기→상세→목록으로 한 단계씩 복귀, 한 번 더로 홈. 상세에서 홈 탭을 누르면 스택이 비고 이후 뒤로가기로 커미션이 되살아나지 않음까지 확인. 실제 iOS 스와이프 체감은 사용자 실기기 확인 예정.
- **남은 것**: 채팅(리스트·채팅방)에 동일 패턴 적용 예정(사용자가 "커미션 먼저 확인 후 채팅"으로 순서 지정).
