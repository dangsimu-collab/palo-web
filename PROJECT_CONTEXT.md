...
- **글쓰기 버튼 부각(2026-08-07, 사용자 요청)**: 확인해보니 **PC는 이미 그라데이션 버튼**이라 충분했고, **모바일 하단 탭의 글쓰기만 다른 탭과 색·크기가 완전히 동일**해 구분이 전혀 없었다(아이콘 색까지 `--muted`로 같음). **시안 3번의 반복 끝에 최종안 = '칸 채우기'**: ①아이콘에 브랜드 그라데이션 배지 → ②배지를 키우고 3px 띄움 → ③**사용자가 방향 전환 요청**('아이콘 말고 칸의 색을 채우는 형태로') → 배지를 걷어내고 탭 칸 자체를 옅은 브랜드색으로 채움. **최종 구현**: `.tab.write::before`로 `inset:0 5px`·`border-radius:14px`·`--brand-soft` 배경 + `#f0d5e3` 테두리를 깔고, 글자·아이콘 색은 `--brand-2`. **배경을 가상 요소로 만든 이유** — 실제 요소에 padding/배경을 주면 레이아웃이 밀린다. ⚠️ 실제로 겪은 문제: 시안 ①에서 `padding:6px`로 배지를 키웠더니 **글쓰기 라벨만 6px 아래로 내려가 줄이 어긋났다**(43 vs 49). 음수 margin으로 자리를 되돌려 해결했었고, 최종안은 가상 요소라 이 문제 자체가 없다. 검증(모바일 375px): 탭바 65px·탭 5개 58px·**라벨 5개 전부 43px·아이콘 상단 5개 전부 14px**로 완전 정렬, 칸 배경 `rgb(247,230,239)`·테두리 `rgb(240,213,227)`, 글자색 글쓰기 `rgb(209,96,143)` vs 나머지 `rgb(162,148,160)`, 데스크톱 영향 없음(하단바 미표시), `next build` 통과.
