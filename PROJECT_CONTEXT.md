...
- **날짜 입력란 디자인 통일 (2026-08-01)**: 작업 사례 폼의 `input[type="date"]`(`.cm-reg-input`)가 iOS/웹킷 네이티브 스타일 때문에 다른 입력란과 안 어울리던 문제 → `globals.css`에 `.cm-reg-input[type="date"]{-webkit-appearance:none;appearance:none;min-height:50px;line-height:normal;text-align:left;background:var(--surface)}` + `::-webkit-date-and-time-value{text-align:left;margin:0}` + `::-webkit-calendar-picker-indicator{opacity:.55}` 추가. dev 확인: 테두리·반경·배경·패딩·정렬 텍스트 입력란과 일치(높이만 Chromium 50 vs 52로 2px차, 무시 가능·`min-height`는 iOS 찌그러짐 방지용).
