...
- **커미션 작업물 명칭 재정리 + 추천 점수 안내 추가(2026-08-07, 사용자 요청)**:
  ...
  "추천 점수의 '작업물 활발도'(전체의 12%)에 반영, 5개까지 개수 반영, 최근일수록 높고 60일이면 0".
  문구의 수치는 이 문서 690줄의 점수 규칙(빈도 min(개수,5)/5×100 ×0.5 + 최신도 60일 선형감쇠 ×0.5, 가중치 0.12)과 맞춘 것 —
  **가중치나 감쇠 기간을 바꾸면 이 안내 문구도 같이 고쳐야 한다.**
