...
- **글쓰기 버튼 부각(2026-08-07, 사용자 요청 "과하지 않게")**: 확인해보니 **PC는 이미 그라데이션 버튼**이라 충분히 눈에 띄었고, **모바일 하단 탭의 글쓰기만 다른 탭과 색·크기가 완전히 동일**해서 전혀 구분되지 않았다(실측: 아이콘 색까지 `--muted`로 같음). **처리**: 하단 탭 글쓰기 아이콘에만 브랜드 그라데이션 배경 + 흰 아이콘, 라벨은 `--muted`→`--ink-2`로 살짝만 진하게. FAB(가운데 큰 원형 버튼)는 과하다고 판단해 **탭 크기·라벨·바 높이는 그대로 유지**. ⚠️ **작업 중 발견한 정렬 문제**: 처음엔 `padding:6px`로 배경을 키우고 탭 세로 여백을 줄여 보정했는데, 이러면 아이콘 아래 6px이 라벨을 밀어 **글쓰기 라벨만 6px 아래로 내려가 다른 탭과 어긋났다**(실측 43 vs 49). → `padding:4px` + **같은 크기의 음수 `margin:-4px`**로 바꿔 **배경만 키우고 레이아웃 자리는 24px 그대로** 유지. **→ 한 단계 더 강조(사용자 요청)**: 배경 32×32→**38×38**, 그림자 `0 3px 9px/.32`→`0 5px 14px/.45`, 선 굵기 2.2→2.4, 그리고 위아래 margin을 **비대칭(-10/-4)**으로 줘 배경만 **3px 띄웠다**(합은 -14로 같아서 차지하는 자리는 24px 그대로). 버튼이 살짝 떠 보이지만 라벨 줄은 안 흐트러진다. 검증(모바일 375px): 탭바 65px·탭 5개 모두 58px 유지, **라벨 5개 전부 43px에 정렬**, 배경 38×38(바 위 4px·라벨과 1px 여유), 데스크톱에선 하단바 미표시·PC 글쓰기 버튼 영향 없음, `next build` 통과.
