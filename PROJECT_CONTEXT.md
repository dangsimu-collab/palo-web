...
- **하단 탭바: 떠 있는 유리 알약([1][2] 완료, 2026-08-08)**: 8/8 새벽에 되돌렸던 시도의 재작업.
  ...
  **웹에서 같게 만드는 두 가지 길**:
  ① 본문을 별도 스크롤 상자로 옮기기(네이티브와 동일한 구조) — **`window.scrollTo` 51곳·`position:sticky` 10곳·
     scroll 리스너 4곳을 모두 손봐야 해 위험이 크다 → 채택하지 않음.**
  ② **끊긴 관성을 우리가 이어받아 계속 굴리기(채택)**: 스크롤 이벤트마다 속도(px/ms)를 재 두고,
     동그라미를 누르는 순간 그 속도로 `requestAnimationFrame` 루프를 돌려 `window.scrollTo`로 이어 굴린다.
     감쇠 `0.994^dt`, 끝에 닿거나 0.02px/ms 미만이면 종료. 사용자가 다른 곳을 만지거나 휠을 굴리면 즉시 양보.
     우리가 만든 스크롤은 `touchmove`/`wheel`이 없어 접기 판정에 잡히지 않는다(그대로 펼쳐진 채 유지).
  ⚠️ **처음엔 화면이 그대로 멈췄다. 원인 둘:**
  ⓐ **`html{scroll-behavior:smooth}`가 전역으로 걸려 있어** 매 프레임의 `scrollTo(0,pos)`가 전부
     '부드럽게 이동' 애니메이션으로 바뀌어 서로를 덮어썼다 → **`scrollTo({top,behavior:"auto"})`** 로 명시해야 한다.
  ⓑ **손가락이 닿아 있는 동안의 프로그램 스크롤은 iOS가 무시한다** → 누를 때는 속도만 챙겨 두고(`pendingGlide`),
     **손을 떼는 순간(`pointerup`/`touchend`)** 이어받아 굴린다(600ms 내에만 유효).
  ⚠️ 속도 측정에 `dt<200ms` 가드가 있다 — **자동화 창은 타이머가 강제 지연돼 이 조건을 못 맞춘다.**
     검증하려면 `setTimeout` 대신 **busy-wait로 실제 간격(~80ms)** 을 만들어야 한다.
