...
- **새 진입점 "커미션" 버튼**: 모바일 하단 탭바(홈/게시판/**커미션**/글쓰기/내 정보 — "게시판"과 "글쓰기" 사이에 추가)와 데스크톱 헤더(글쓰기 버튼 왼쪽에 보조 스타일 버튼)에 추가, `openCommissionHub('trade')` 호출.
- **커미션 화면**: `renderList()`에 `isCommission` 분기를 추가해서, `state.board`가 `trade`/`review`이고 검색 중이 아닐 때는 기존 게시판 제목(h1) 자리에 **"구인구직"/"후기" 탭 버튼**(`switchCommissionTab()`)을 대신 넣고, 정렬·목록형/앨범형 버튼 옆에 **"＋ 글쓰기"** 버튼(`openCommissionWrite()` — 현재 탭에 맞는 게시판이 자동 지정된 채 글쓰기 창이 열림, 기존 `openReviewFor()`와 같은 `pickBoard()` 트릭 재사용)을 추가. 그 아래 목록/앨범/말머리 필터/페이지네이션은 전부 기존 `renderList()` 로직을 그대로 재사용(게시판 코드만 `trade`/`review`로 스코프됨).
