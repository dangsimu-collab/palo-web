...
- **하단 탭바를 떠 있는 알약(pill) 형태로 변경(2026-08-08, 사용자 요청 + 참고 이미지)**:
  바닥에 붙은 반투명 바 → **양옆 13px·아래 12px 띄운 흰색 알약**(border-radius 999px, 그림자로 떠 보이게, max-width 440px).
  선택된 탭만 **아이콘 뒤를 브랜드 그라데이션으로 채우고**(`.ic-wrap`) 아이콘을 흰색·글자를 브랜드색으로.
  아이콘은 **선(stroke) → 꽉 찬(fill) 스타일**로 전부 교체(`fill="currentColor"`, 21px).
  ⚠️ **핵심 함정**: 탭을 띄우면 **'탭이 가리는 높이' ≠ '탭 높이'** 가 된다(아래 여백+안전영역이 더해짐).
  `cmSyncTabbarHeight`가 `getBoundingClientRect().height`를 재고 있어서 그대로 두면 그 여백만큼 글이 가린다.
  → **`innerHeight - rect.top`(화면 바닥부터 탭 윗변까지)** 를 재도록 변경. 이 한 줄로 `--cm-tabbar-h`에 의존하는
  기존 8곳(`.cm-fab`·`.cm-apply-bar`·`.cm-pad`·`.cm-grid`·`.cm-wr`·`.cm-reg` 등)이 전부 자동으로 맞는다.
  본문 가림 방지는 `footer{padding-bottom:calc(var(--cm-tabbar-h) + 20px)}`(모바일 미디어쿼리) 하나로 해결 —
  푸터가 문서 마지막 요소라 이 여백이 곧 스크롤 최하단의 안전 공간이 된다.
  안전영역은 `bottom:calc(env(safe-area-inset-bottom,0px) + 12px)`, PWA(standalone)는 +20px.
  📌 **검증 시 주의**: 이 자동화 환경은 **스크롤이 먹지 않아**(`scrollTo` 후에도 scrollY=0) 스크롤 기반 가림 검사가
  거짓 실패를 낸다. **문서 좌표로 계산**(`요소 아래끝 + scrollY` vs `scrollHeight - 가림높이`)해야 정확하다.
  📌 드래그 취소 검증 시 `tap()` 헬퍼가 click까지 발사하면 안 된다 — 실제 브라우저는 끌면 click을 안 만든다.
  검증(375px): 좌우 13·아래 12px 여백, 가림높이 79px과 실측 일치, 홈/커미션/내정보 모두 여유 20px 이상,
  연타 3번 각 1회·끌어서 취소 0회·정상 탭 1회.
