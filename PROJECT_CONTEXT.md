...
- **Phase 2 삭제 기록(감사 로그)**: `admin_post_deletions` 표(`post_id`/`board`/`title`/`content` 본문 스냅샷/`author_id`/`author_nick`/`admin_id`/`admin_nick`/`reason`/`notified`/`created_at`). RLS는 `chat_admin_access_logs`와 동일 원칙 — **select만 `is_admin()`**, insert/update/delete 정책 없음(=security definer RPC로만 기록, 관리자도 못 고침·못 지움 = 증거 보존). `admin_delete_post` RPC가 삭제 직전에 스냅샷+작성자/관리자 닉네임을 넣음. 뷰어: 내 정보 > 🛡 관리자 메뉴 > "🗑 삭제 기록"(`openAdminDeletionLog`→`renderAdminDeletionLog`, `.del-log*` 스타일, 최근 100건 카드 목록 — 게시판·시각·제목·본문 발췌·작성자·삭제관리자·사유·알림발송여부).
